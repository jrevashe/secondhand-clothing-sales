...
#bring each entry to lower case
#create a defined range of possible values to group each item in
...
def transform_product_season(entry):
    count = 0
    entry = entry.lower()
    #{1: all seasons, 2: autumn/winter, 3: spring/summer}
...
def transform_product_material(entry):
    count = 0
    entry = entry.lower()
    #{1: viscose, 2: cotton, 3: synthetic, 4: polyester, 5: polyamide, 6: wool, 7: leather, 8: silk, 9: denim, 10: suede, 11: khaki, 12: linen, 13: cashmere, 14: velvet, 15: lace, 16: fur, 17: glitter, 18: tweed, 19: vinyl, 20: lycra, 21: spandex, 22: shearling, 23: rubber, 24: plastic, 25: cloth, 26: sponge, 27: animal}
...
def transform_color_clean(entry):
    count = 0
    entry = entry.lower()
    #{1: metallic colors, 2: multicolour, 3: blue, 4: black, 5: brown, 6: green, 7: pink, 8: white, 9: red, 10: grey, 11: yellow, 12: orange, 13: purple, 14: turquoise}
...
def transform_seller_country(entry):
    count = 0
    #{1: europe, 2: asia, 3: north america, 4:south america, 5: africa, 6: oceania, 7: carriabean}
